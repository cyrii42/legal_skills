...
1. **Metadata block** *(factual)*: a header block giving case name and docket (if known), deponent name and role/title, deposition date, date prepared, summary type (facts-only / quick read / full), and a transcript-type/reliability note. The confidentiality legend is not part of the body: the docx builder places it in the Word page header (right-justified, repeating on every page), so you do not need to include it in the Markdown. If you do put a "Privileged & Confidential" line at the top of the Markdown, the builder ignores it to avoid duplication.
