...
- **Identifying facts are held to the same standard as testimony.** Personal names and their spellings, titles, firm affiliations, party roles, dates, docket numbers, and exhibit or Bates identifiers may never be supplied from memory or inference. Copy every proper noun character-for-character from the source (the transcript, an exhibit, or the case materials); never type a name from recollection. If a source does not supply one, leave a visible gap (for example, "[first name not stated in the record]") and never fill it with a plausible-looking value. An invented name is a fabrication in the same category as invented testimony, and no less serious for appearing in a header than in the body.
- **Drafting adds analysis, not facts.** Every factual particular in the finished summary—every name, date, title, quotation, and cite—must come from the transcript, an exhibit, or the case materials. Composing the summary is where you organize and analyze what the sources say; it is never where a new fact enters. If you are about to write a detail that appears in none of your sources, stop and either find it in a source or leave a visible gap.
...
1. **Metadata block** *(factual)*: a header block giving case name and docket (if known), deponent name and role/title, deposition date, date prepared, summary type (facts-only / quick read / full), and a transcript-type/reliability note. The confidentiality legend is not part of the body: the docx builder places it in the Word page header (right-justified, repeating on every page), so you do not need to include it in the Markdown. If you do put a "Privileged & Confidential" line at the top of the Markdown, the builder ignores it to avoid duplication. Source every name in this block—the deponent and any counsel or other participants you name—from the transcript's appearance block and on-record appearances, copied exactly as spelled there; where the transcript gives only a surname, use only the surname.
...
7. **Internal credibility signals** *(factual, context-free)*: bulleted. This section is for what a reader would otherwise miss, and it requires actively hunting: evasions and non-answers, implausibly generic or selective non-recall on matters the witness should command, assumption offered in place of knowledge, and subtle or buried contradictions (cite both passages). Do not restate a headline impeachment already developed in the Overview or a topic section; develop it there. Each entry must name the signal specifically with cites, not gesture at "evasiveness." If, after a genuine search, there are none, say so in one sentence rather than omitting the section.
...
- **Quote-completeness check (every output):** confirm each quotation includes its framing question(s) and any immediately following qualification, and that an admission built across several Q&A pairs is quoted as the whole run. An answer quoted without its question is a defect.
- **Identifier check (every output).** Verify every personal name and its spelling, every title, firm, date, docket number, and exhibit or Bates identifier against the source; check participant names against the transcript's appearance block. Correct any mismatch and flag anything the source does not support. This is separate from, and in addition to, the citation check, and it runs even in facts-only mode.
- **Adversarial self-review (full work-product summary).** Make a dedicated skeptical pass over the finished draft: treat every name, quotation, and cite as wrong until you re-locate it in the source, and assume you have introduced at least one unsupported specific, then hunt for it. Fix or flag whatever you cannot anchor.
...
- The builder applies the house style: Times New Roman 12pt; bold, section-numbered headings at body size; 1-inch margins; centered footer page numbers; left-aligned, single-spaced body; and the "Privileged & Confidential - Attorney Work Product" legend as a bold, right-justified Word page header that repeats on every page.
- **No Obsidian wikilinks, and no wikilink-based cross-references section.** The deliverable is a Word document, where `[[Note]]` / `[[Note|Display]]` render as literal bracket text and point at vault notes no external reader can open. Do not add a "Cross-references" (or similar) section, and do not use wikilinks anywhere in the summary. Reference the transcript and any related document by plain name in the metadata block or prose. The builder strips wikilink markup to plain text as a backstop (`[[a/b#c|Display]]` becomes `Display`; a bare `[[a/b#c]]` becomes `b`), but do not rely on it: write plain references in the first place.
- **Typographic quotation marks are mandatory in the .docx.** The builder automatically converts straight quotes (`"` and `'`) in the Markdown to curly/smart quotes (“ ” ‘ ’) and apostrophes when it renders the Word document, leaving inline-code and fenced-code spans (Bates labels, Westlaw asterisk cites) untouched. Write the Markdown with ordinary straight quotes (better for the vault and for searching); do not hand-curl them. If you generate the .docx by any path other than the bundled builder, you must still apply the same smart-quote conversion so the delivered Word document never contains straight quotes or apostrophes.
